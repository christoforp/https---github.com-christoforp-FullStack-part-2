...
const getAllValues =  async () => { 
    const request = axios.get(baseUrl)  // Variables "baseUrl" has initialized in row 2 
    const response = await request;
    return response.data; // When function has conducted, so the current data is saved under the "return response.data", that we can use that variables data in side, if this is not used, then result would be error-
...
const deleteValue = async (id) => { // We initalize also "id" variables use inside that function, if this not would be then it would be error. 
    const request = axios.delete(`${baseUrl}/${id}`) // Variables "id" has initialized in row 2 
    const response = await request;
    return response.data;  // When function has conducted, then the current data saved  under "response.data ", that we can use its variables data in side, if this would not be used then it would be error! 
}
